...
def generate_launch_description():
    ...
    package_name = FindPackageShare('ur5_robot_gripper')

    ld.add_action(DeclareLaunchArgument(name='jsp_gui', default_value='false', choices=['true', 'false'],
                                        description='Flag to enable joint_state_publisher_gui'))

    default_rviz_config_path = PathJoinSubstitution([package_name, 'udf', 'urdf.rviz'])
    ...
    ld.add_action(IncludeLaunchDescription(
        PathJoinSubstitution([package_name, 'launch', 'description.launch.py']),
        launch_arguments={
